...
public class G1opAgent : Agent
{
    ...
    public override void Initialize()  //初始化，只在程序刚运行的那一刻执行一次
    {
    ...
    public override void OnEpisodeBegin()  //当机器人摔倒或完成任务重置时执行。它会把机器人传送回起点，并清空速度
    {
        tp = 0;
        ...
        wr=0;
        cr=0.2f;
       	GetComponent<RobotSocketReceiver>().NotifyReset();
    }

    // 软重置：仅恢复关节链与控制器状态到稳定初始值，不重置场景/位置。
    // Why: RespawnAtLastWaypoint 只 TeleportRoot 根节点，但人形关节仍处于摔倒前的扭曲姿态，
    // 残留的关节速度与 PD 控制器目标会让机器人重生后立刻再次失稳摔倒。
    public void ResetToStablePose()
    {
        tp = Random.Range(0, 2 * T1);
        tq = 0;
        tt = 0;
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)//最最核心
    {
        for (int i = 0; i < 12; i++) utotal[i] = 0;
